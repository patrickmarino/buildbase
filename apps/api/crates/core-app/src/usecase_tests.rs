...
async fn login_rejects_wrong_password() {
    let w = World::new();
    let err = auth(&w)
        .login(w.org_id, "elena@madespace.co", "nope")
        .await
        .unwrap_err();
...
async fn login_rejects_deactivated_user() {
    ...
    users(&w)
        ...
        .unwrap();
    let err = auth(&w)
        .login(w.org_id, "daniel@madespace.co", "memberpw")
        .await
        .unwrap_err();
    assert!(matches!(err, AppError::Unauthorized));
}
...
async fn invite_rejects_duplicate_email() {
    ...
    let ctx = w.ctx_for(w.owner_id).await;
    let err = users(&w)
        .invite(&ctx, "tomas@madespace.co", "member", None)
        .await
        .unwrap_err();
...
async fn invite_rejects_invalid_email() {
    ...
    let ctx = w.ctx_for(w.owner_id).await;
    assert!(users(&w)
        .invite(&ctx, "not-an-email", "member", None)
        .await
        .is_err());
}
...
async fn member_cannot_invite() {
    ...
    let err = users(&w)
        .invite(&ctx, "x@madespace.co", "member", None)
        .await
        .unwrap_err();
...
async fn change_role_revokes_sessions_and_audits_once() {
    ...
    let updated = users(&w)
        .change_role(&ctx, w.member_id, viewer_role)
        .await
        .unwrap();
...
async fn last_admin_cannot_be_demoted() {
    ...
    users(&w)
        .set_status(&ctx, w.admin_id, UserStatus::Deactivated)
        .await
        .unwrap();
    let ctx = w.ctx_for(w.owner_id).await;
    ...
    let err = users(&w)
        .change_role(&ctx, w.owner_id, member_role)
        .await
        .unwrap_err();
    assert!(matches!(
        err,
        AppError::Domain(DomainError::LastAdminProtected(_))
    ));
}
...
async fn last_admin_cannot_be_deactivated() {
    ...
    let ctx = w.ctx_for(w.owner_id).await;
    let err = users(&w)
        .set_status(&ctx, w.owner_id, UserStatus::Deactivated)
        .await
        .unwrap_err();
    assert!(matches!(
        err,
...
async fn deactivating_member_revokes_their_sessions() {
    ...
    let ctx = w.ctx_for(w.owner_id).await;
    users(&w)
        .set_status(&ctx, w.member_id, UserStatus::Deactivated)
        .await
        .unwrap();
    assert_eq!(w.session_count(), 0);
}
...
async fn admin_cannot_assign_owner() {
    ...
    let err = users(&w)
        .change_role(&ctx, w.member_id, owner_role)
        .await
        .unwrap_err();
    assert!(matches!(
        err,
...
async fn cycle_cell_changes_state_and_audits() {
    ...
    let cell = perms(&w)
        .cycle_cell(&ctx, "users.invite", admin_role)
        .await
        .unwrap();
...
async fn cycle_cell_rejects_locked_cell() {
    ...
    let err = perms(&w)
        .cycle_cell(&ctx, "org.delete", admin_role)
        .await
        .unwrap_err();
    ...
    assert_eq!(
        w.audit_count(),
        before_audits,
        "nothing audited on a locked cell"
    );
}
...
async fn create_custom_role_copies_base_cells() {
    ...
    let role = roles(&w)
        .create_custom(&ctx, "Stager", viewer)
        .await
        .unwrap();
    ...
    let matrix = perms(&w)
        .get_matrix(&w.ctx_for(w.owner_id).await)
        .await
        .unwrap();
...
async fn create_custom_role_rejects_duplicate_name() {
    ...
    let viewer = w.roles_by_key["viewer"];
    assert!(roles(&w)
        .create_custom(&ctx, "Admin", viewer)
        .await
        .is_err());
}
...
async fn create_key_returns_token_once_and_stores_hash_only() {
    ...
    let created = keys(&w)
        .create(
            &ctx,
            "Booking sync",
            vec![Scope::UsersView, Scope::AuditView],
        )
        .await
        .unwrap();
...
async fn revoke_key_sets_status_and_audits() {
    ...
    let ctx = w.ctx_for(w.owner_id).await;
    let created = keys(&w)
        .create(&ctx, "Temp", vec![Scope::UsersView])
        .await
        .unwrap();
...
async fn ownership_transfer_initiate_then_accept() {
    ...
    org(&w)
        .transfer_ownership(&owner_ctx, w.admin_id)
        .await
        .unwrap();
    // admin accepts
...
async fn audit_search_and_export() {
    ...
    users(&w)
        .invite(&ctx, "z@madespace.co", "member", None)
        .await
        .unwrap();
    let ctx = w.ctx_for(w.owner_id).await;
    ...
    let csv = audit(&w)
        .export_csv(&ctx, AuditQuery::default())
        .await
        .unwrap();
